# 🟢 Scribelia Status

[![Status](https://status.scribelia.com/badge.svg)](https://status.scribelia.com)

> État des services Scribelia en temps réel

**Statut actuel:** 🟢 Tous les systèmes opérationnels

*Dernière mise à jour: 28/01/2026 21:00:29*

---

## 📊 État des services

| Service | État | Temps de réponse | Uptime (24h) | Uptime (7j) | Uptime (30j) |
|---------|------|------------------|--------------|-------------|--------------|
| **API** | ✅ Opérationnel | 789ms | 64.13% | 70.57% | 75.03% |
| **Website** | ✅ Opérationnel | 239ms | 100% | 99.59% | 99.77% |

---

## 📈 Historique récent

### 🚨 Incidents en cours

- **API** - ⚠️ Dégradé depuis 1h 40m
- **API** - ⚠️ Dégradé depuis 6h 50m
- **API** - ⚠️ Dégradé depuis 3j 2h
- **API** - ❌ Hors service depuis 7j 5h
- **API** - ⚠️ Dégradé depuis 7j 5h
- **API** - ❌ Hors service depuis 16j 5h
- **API** - ⚠️ Dégradé depuis 16j 7h

### ✅ Incidents résolus récemment

- **API** - 28/01/2026 - Résolu en 1m
- **API** - 28/01/2026 - Résolu en 6m
- **API** - 28/01/2026 - Résolu en 5m
- **API** - 28/01/2026 - Résolu en 29m
- **API** - 28/01/2026 - Résolu en 8m

---

## 📋 Services monitorés

### API
- **URL:** `https://api.scribelia.com/health`
- **Description:** API Backend
- **Temps de réponse moyen (24h):** 693ms
- **Temps de réponse moyen (7j):** 643ms

### Website
- **URL:** `https://scribelia.com`
- **Description:** Frontend Application
- **Temps de réponse moyen (24h):** 317ms
- **Temps de réponse moyen (7j):** 347ms

---

## 🔧 À propos

Cette page de statut est mise à jour automatiquement toutes les 5 minutes via GitHub Actions.

- 📁 [Voir les données brutes](./data/status.json)
- 🌐 [Page de statut](https://status.scribelia.com)

---

*Propulsé par [Scribelia Status Monitor](https://github.com/scribelia-org/scribelia-status)*
