# 🟢 Scribelia Status

[![Status](https://status.scribelia.com/badge.svg)](https://status.scribelia.com)

> État des services Scribelia en temps réel

**Statut actuel:** 🟢 Tous les systèmes opérationnels

*Dernière mise à jour: 01/02/2026 10:47:36*

---

## 📊 État des services

| Service | État | Temps de réponse | Uptime (24h) | Uptime (7j) | Uptime (30j) |
|---------|------|------------------|--------------|-------------|--------------|
| **API** | ✅ Opérationnel | 707ms | 67.39% | 64.34% | 73.44% |
| **Website** | ✅ Opérationnel | 337ms | 100% | 99.51% | 99.79% |

---

## 📈 Historique récent

### 🚨 Incidents en cours

- **API** - ❌ Hors service depuis 15h 25m
- **API** - ⚠️ Dégradé depuis 21h 27m
- **API** - ⚠️ Dégradé depuis 23h 23m
- **API** - ❌ Hors service depuis 1j 2h
- **API** - ❌ Hors service depuis 1j 14h
- **API** - ⚠️ Dégradé depuis 1j 15h
- **API** - ❌ Hors service depuis 1j 15h
- **API** - ❌ Hors service depuis 1j 17h
- **API** - ❌ Hors service depuis 2j 16h
- **API** - ❌ Hors service depuis 2j 17h
- **API** - ⚠️ Dégradé depuis 3j 15h
- **API** - ⚠️ Dégradé depuis 3j 20h
- **API** - ⚠️ Dégradé depuis 6j 16h
- **API** - ❌ Hors service depuis 10j 18h
- **API** - ⚠️ Dégradé depuis 10j 18h
- **API** - ❌ Hors service depuis 19j 19h
- **API** - ⚠️ Dégradé depuis 19j 21h

### ✅ Incidents résolus récemment

- **API** - 01/02/2026 - Résolu en 21m
- **API** - 01/02/2026 - Résolu en 8m
- **API** - 01/02/2026 - Résolu en 3m
- **API** - 01/02/2026 - Résolu en 2m
- **API** - 01/02/2026 - Résolu en 1m

---

## 📋 Services monitorés

### API
- **URL:** `https://api.scribelia.com/health`
- **Description:** API Backend
- **Temps de réponse moyen (24h):** 630ms
- **Temps de réponse moyen (7j):** 659ms

### Website
- **URL:** `https://scribelia.com`
- **Description:** Frontend Application
- **Temps de réponse moyen (24h):** 252ms
- **Temps de réponse moyen (7j):** 319ms

---

## 🔧 À propos

Cette page de statut est mise à jour automatiquement toutes les 5 minutes via GitHub Actions.

- 📁 [Voir les données brutes](./data/status.json)
- 🌐 [Page de statut](https://status.scribelia.com)

---

*Propulsé par [Scribelia Status Monitor](https://github.com/scribelia-org/scribelia-status)*
